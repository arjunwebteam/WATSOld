...
namespace WATS.BLL
{
    public class Donors
    {
        ...
        public List<WATS.Entities.Donors> GetDonorsListByVariable(string Search, string Sort, int PageNo, int Items, ref int Total)
        {
            List<WATS.Entities.Donors> lstDonors = new List<WATS.Entities.Donors>();
            DataTable dt = _Donors.GetDonorsListByVariable(Search, Sort, PageNo, Items, ref Total);
            if (dt.Rows.Count == 0 && PageNo != 0)
            {
                dt = _Donors.GetDonorsListByVariable(Search, Sort, PageNo - 1, Items, ref Total);
            }
            if (dt.Rows.Count != 0)
            {
